                    ...
                    {table.map((tableItem: Table, index: number) => (
                        <tr key={index} className="text-center border-b odd:bg-slate-200">
                            ...
                            <td className="py-2 px-4 font-bold flex text-left whitespace-nowrap">
                                <Image width={24} className="mr-3" height={10} src={tableItem.team.crest} alt={`escudo do ${tableItem.team.shortName}`} />
                            ...
                            <td className="mobile-hidden py-2 px-4 whitespace-nowrap">{tableItem.goalsFor - tableItem.goalsAgainst}</td>
                            <td className="mobile-hidden py-2 px-4 whitespace-nowrap">{(tableItem.won / tableItem.playedGames * 100).toFixed(2)}%</td>
                            <td className="py-2 px-4 flex justify-center gap-1 whitespace-nowrap">{tableItem.formerMatches.slice(-5).map(result => processFormerMatches(result))}</td>
                        </tr>
                    ))}
                </tbody>
            </table>
